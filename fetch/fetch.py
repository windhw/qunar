...
def get_price(query_data):
    ...
    #queryID  = re.search(r'"queryID"\s*:\s*"([^"]+)"',html3).group(1)
    queryID = html3_dict["queryID"]
    #req4_serverIP = re.search(r'"serverIP"\s*:\s*"([^"]+)"',html3).group(1)
    ...
    #"serverIP": "JRFrkvvjnOT5ZBC7JrJzcw0Qvd9/F8Wph4thE9v+RdIlboiP4ea3Dw==",
    req4_params = {
    ...
    returnDic = {}
    while True:
        #log ( "[%10s] Loop %d fetch .." % (stage, fetch_cnt) )
        detect_stop()
        if stage == "price" :
    ...
    if html4_dict.has_key("priceInfo"):
                for (n, v) in html4_dict["priceInfo"].items():
                    ...
                    if resultDic[n].get("ignore"):
                        continue
                    #resultDic[n].setdefault("priceInfo", v)
